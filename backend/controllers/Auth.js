...
exports.signup = async (req, res) => {
    try {
        ...
        const { name, email, password, isAdmin = false } = req.body; // Destructure `isAdmin` with a default `false`

        // Hash the password
        const hashedPassword = await bcrypt.hash(password, 10);

        // Create new user with explicit `isAdmin` and hashed password
        ...
        // Get secure user info without sensitive data
        const secureInfo = sanitizeUser(createdUser);

        // Generate JWT token
        const token = generateToken(secureInfo);

        // Send JWT token in response cookies
