...
export const AdminDashBoard = () => {
  ...
  return (
    ...
    <motion.div style={{position:"fixed",backgroundColor:"white",height:"100vh",padding:'1rem',overflowY:"scroll",width:is500?"100vw":"30rem",zIndex:500}}  variants={{show:{left:0},hide:{left:-500}}} initial={'hide'} transition={{ease:"easeInOut",duration:.7,type:"spring"}} animate={isProductFilterOpen===true?"show":"hide"}>
        ...
        <Stack rowGap={2} mt={4} >
            <Typography sx={{cursor:"pointer"}} variant='body2'>Diary</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Vegetable</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Fruits</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Breakfast Items</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Wedding Leftover food</Typography>
        </Stack>
                ...
                products.map((product)=>(
                    <Stack>
                        <Stack sx={{opacity:product.isDeleted?.7:1}}>
                            <ProductCard key={product._id} id={product._id} title={product.title} thumbnail={product.thumbnail} expiry={product.expiry} brand={product.brand.name} price={product.price} isAdminCard={true}/>
                        </Stack>
